...
parser.add_argument('--devices', type=str, default='2', help='gpu device ids')
parser.add_argument('--print_num', type=int, default='50', help='gap of print evaluations')
parser.add_argument('--test_time', type=int, default='3', help='number of test times')
...
def main():
    ...
    for data_seed in range(1, args.test_time + 1):
        ...
        # Build model
        SCAE = SCAEMultiView(config)
        optimizer = torch.optim.Adam(
            itertools.chain(SCAE.autoencoder1.parameters(), SCAE.autoencoder2.parameters(),
                            SCAE.autoencoder3.parameters(),
                            SCAE.a2b.parameters(), SCAE.b2a.parameters(),
                            SCAE.a2c.parameters(), SCAE.c2a.parameters(),
                            SCAE.b2c.parameters(), SCAE.c2b.parameters()
                            ),
            lr=config['training']['lr'])

        logger.info(SCAE.autoencoder1)
        logger.info(SCAE.a2b)
        logger.info(optimizer)

        SCAE.autoencoder1.to(device), SCAE.autoencoder2.to(device), SCAE.autoencoder3.to(device)
        SCAE.a2b.to(device), SCAE.b2a.to(device)
        SCAE.b2c.to(device), SCAE.c2b.to(device)
        SCAE.a2c.to(device), SCAE.c2a.to(device)

        if config['type'] == 'CG':
            acc, precision, f_measure = SCAE.train_completegraph_supervised(config, logger, accumulated_metrics,
                                                                           x1_train, x2_train, x3_train, x1_test,
                                                                           x2_test, x3_test, labels_train,
                                                                           labels_test, mask_train, mask_test,
                                                                           optimizer, device)
        elif config['type'] == 'CV':
            acc, precision, f_measure = SCAE.train_coreview_supervised(config, logger, accumulated_metrics,
                                                                      x1_train, x2_train, x3_train, x1_test,
                                                                      x2_test, x3_test, labels_train,
                                                                      labels_test, mask_train, mask_test,
                                                                      optimizer, device)
