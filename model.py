...
from loss import instance_contrastive_Loss, category_contrastive_loss, GMRFLoss
...
class SCAE():
    ...
    def train_clustering(self, config, logger, accumulated_metrics, x1_train,
        ...
        for k in range(epochs_total):
            ...
            for batch_x1, batch_x2, batch_No in next_batch(X1, X2, batch_size):
                ...
                loss_vic = self.contrast_model(self.drop_feature(z_1, 0.2), self.drop_feature(z_2, 0.5))
                ...
                loss_gmr = (loss_gmr1 + 0.1 * loss_gmr2) / 2
                ...
                all_loss = loss_icl + reconstruction_loss * config['training']['lambda2'] #+ 0.05* loss_gmr + 0.05*loss_vic
    ...
    def train_HAR(self, config, logger, accumulated_metrics, train_data, optimizer, device):
        ...
        return max(accumulated_metrics['RGB']), max(accumulated_metrics['Depth']), max(accumulated_metrics['RGB-D']), max(accumulated_metrics['onlyRGB']), max(accumulated_metrics['onlyDepth'])
